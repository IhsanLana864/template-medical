                    ...
                    <form action="{{ route('admin.fasilitas.update', $fasilitas->id) }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-12 mb-4">
                                ...
                                @if ($fasilitas->gambar)
                                    <div class="mt-3" id="current_gambar_container">
                                        <p>Gambar Saat Ini:</p>
                                        <img src="{{ asset('storage/' . $fasilitas->gambar) }}" alt="Gambar fasilitas Saat Ini" style="max-width: 300px; height: auto; border: 1px solid #ddd; padding: 5px;">
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="col-12 mb-4">
                                ...
                                {{-- Menggunakan old() untuk error validasi, fallback ke data $fasilitas --}}
                                <input type="text" class="form-control mb-2" name="judul" value="{{ old('judul', $fasilitas->judul) }}" placeholder="Judul Gambar">
                                @error('judul')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-12 mb-4">
                                ...
                                {{-- Menggunakan old() untuk error validasi, fallback ke data $fasilitas --}}
                                <input type="text" class="form-control mb-2" name="keterangan" value="{{ old('keterangan', $fasilitas->keterangan) }}" placeholder="Keterangan Gambar">
                                @error('keterangan')
                                    <div class="text-danger">{{ $message }}</div>
                                @enderror
                            </div>
...
<script>
    $(document).ready(function() {
        ...
        @if ($fasilitas->gambar)
            ...
            var initialGambarSrc = "{{ old('gambar') ? asset('storage/' . old('gambar')) : asset('storage/' . $fasilitas->gambar) }}";
